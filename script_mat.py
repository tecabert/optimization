...
def plot1(original, reconstructed):
    ...
    # Compute sorted ranks of normalized original and reconstructed non-zero values
    original_ranks = original_norm
    reconstructed_ranks = reconstructed_norm

    # Create QQ plot
    fig, ax = plt.subplots()
    ax.scatter(reconstructed_ranks, original_ranks)
    ax.plot([0, 1], [0, 1], 'k--')
...
def main():
    ...
    tmax = 30
    lambda_ = .1 * np.linalg.norm(phi.adjoint(y), np.infty)
